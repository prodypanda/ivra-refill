...
/// A class representing SupabaseIvraRepository.
///
/// Provides data structure and operations for SupabaseIvraRepository.
class SupabaseIvraRepository implements IvraRepository {
  ...
  static String _resourceFamily(String key) {
    ...
    final looksLikeId =
        suffix.length >= 3 && RegExp(r'^[0-9a-fA-F-]+$').hasMatch(suffix);
  ...
  static Object? _readCacheEnvelope(
    String? cached, {
  ...
  @visibleForTesting
  static Object? readCacheEnvelopeForTest(
    String? cached, {
    String? expectedVersion,
    Duration? maxAge,
  }) =>
  ...
  static List<Map<String, dynamic>> _decodeMapList(Object? decoded) {
    ...
    throw StateError(
      'Cached JSON payload is not a List (got ${decoded.runtimeType}).',
    );
  }
  ...
  static Map<String, dynamic> _decodeMap(Object? decoded) {
    ...
    throw StateError(
      'Cached JSON payload is not a Map (got ${decoded.runtimeType}).',
    );
  }
  ...
  Future<void> _clearRefillEventsCache() async {
    ...
    final keys = prefs
        .getKeys()
        .where((k) => k.startsWith('cache_recent_refill_events_'))
        .toList();
    for (final key in keys) {
      await prefs.remove(key);
    }
  ...
  Future<void> _clearInventoryCache() async {
    final prefs = await SharedPreferences.getInstance();
    final keys =
        prefs.getKeys().where((k) => k.startsWith('cache_inventory_')).toList();
    for (final key in keys) {
      await prefs.remove(key);
    }
  }
  ...
  Future<void> updateUserProfile({
    ...
    await _auditService.logAction(
      ...
      details: {'user_id': userId, 'full_name': fullName, 'role': role?.value},
    );
  }
  ...
  Future<String> updateUserAvatar({
    ...
    await _client.rpc(
      'update_user_avatar',
      params: {'p_user_id': userId, 'p_avatar_url': publicUrl},
    );
  ...
  Future<List<HousekeeperAllocation>> fetchHousekeeperAllocations({
    ...
    var query =
        _client.from('housekeeper_allocations').select('*, products(*)');
    ...
    return rows
        .map<HousekeeperAllocation?>((row) {
          if (row['products'] == null) {
            AppLogger.error(
              'Missing product in housekeeper allocation row: $row',
            );
            return null;
          }
          ...
          return HousekeeperAllocation(
            ...
            openBidonVolumeLeftMl: asDouble(
              flattened['open_bidon_volume_left_ml'],
            ),
          );
        })
        .whereType<HousekeeperAllocation>()
        .toList();
  }
  ...
  Future<List<HousekeeperStockEvent>> fetchHousekeeperStockEvents({
    ...
    return rows
        .map<HousekeeperStockEvent?>((row) {
          if (row['products'] == null) {
            AppLogger.error(
              'Missing product in housekeeper stock event row: $row',
            );
            return null;
          }
          final productMap = row['products'] as Map<String, dynamic>? ?? {};
          ...
          return HousekeeperStockEvent(
            ...
            housekeeperId: asString(flattened['housekeeper_id']),
            product: _joinedProductFromMap(flattened),
            eventType: housekeeperStockEventTypeFromDb(
              asString(flattened['event_type']),
            ),
            ...
            createdAt: DateTime.tryParse(
                  asString(flattened['created_at']),
                )?.toLocal() ??
                DateTime.now(),
            roomProductId: flattened['room_product_id'] == null
                ? null
                : asString(flattened['room_product_id']),
            roomNumber:
                roomMap == null ? null : asString(roomMap['room_number']),
            notes: flattened['notes'] == null
                ? null
                : asString(flattened['notes']),
          );
        })
        .whereType<HousekeeperStockEvent>()
        .toList();
  }
  ...
  Future<List<RefillEvent>> recentRefillEvents({String? hotelId}) async {
    var query = _client.from('refill_events').select(
          '*, profiles!refill_events_performed_by_profile_fkey(full_name)',
        );
    if (hotelId != null) query = query.eq('hotel_id', hotelId);
    final rows = await _fetchWithCache(
  ...
  Future<void> inviteTeamMember({
    ...
    await _auditService.logAction(
      'Invited team member',
      details: {'email': email, 'role': role, 'hotel_id': hotelId},
    );
  }
  ...
  Future<void> acceptTeamInvitation({required String token}) async {
    try {
      await _client.rpc('accept_team_invitation', params: {'p_token': token});
    } catch (e) {
  ...
  Future<void> recordRefill({
    ...
    await _auditService.logAction(
      'Recorded refill',
      details: {'room_product_id': roomProductId},
    );
  }
  ...
  Future<void> undoRefill({
    ...
    await _auditService.logAction(
      'Undid refill',
      details: {'refill_event_id': refillEventId},
    );
  }
  ...
  Future<String?> submitChangeRequest({
    ...
    await _auditService.logAction(
      ...
      details: {'target_table': targetTable, 'target_id': targetId},
    );
    return result;
  }
  ...
  Future<void> recordStockAdjustment({
    ...
    await _auditService.logAction(
      ...
      details: {'hotel_id': hotelId, 'product_id': productId},
    );
  }
  ...
  Future<void> approveRequest({
    ...
    await _client.rpc(
      ...
      params: {'p_request_id': approvalRequestId, 'p_notes': notes},
    );
    await _clearRefillEventsCache();
    await _clearInventoryCache();
    await _auditService.logAction(
      'Approved change request',
      details: {'request_id': approvalRequestId},
    );
  }
  ...
  Future<int> refreshSmartAlerts({String? hotelId}) async {
    ...
    await _auditService.logAction(
      'Refreshed smart alerts',
      details: {'hotel_id': hotelId, 'created_count': created},
    );
    return created;
  }
  ...
  Future<void> resolveAlert({required String alertId}) async {
    await _client.rpc('resolve_alert', params: {'p_alert_id': alertId});
  ...
  Future<void> assignUserHotel({
    ...
    await _client.rpc(
      'assign_user_hotel',
      params: {'p_user_id': userId, 'p_hotel_id': hotelId},
    );
    await _auditService.logAction(
      'Assigned user to hotel',
      details: {'user_id': userId, 'hotel_id': hotelId},
    );
  }
  ...
  Future<Map<String, Set<String>>> fetchRolePermissions() async {
    ...
    final Map<String, Set<String>> matrix = {for (final r in roles) r: {}};
  ...
  Future<List<String>> fetchAllPermissions() async {
    ...
    final permissions =
        data.map<String>((row) => row['permission'] as String).toSet().toList();
    permissions.sort();
    return permissions;
  }
  ...
  Future<void> addProductToRoom({
    ...
    final floorResult = await _client
        .from('floors')
        .select('id')
        .eq('hotel_id', hotelId)
    ...
    if (deductFromHousekeeperId != null) {
      ...
      final invResult = await _client
          ...
          .maybeSingle();
      final currentStock =
          invResult != null ? asInt(invResult['full_bottles']) : 0;
    ...
    final roomProductResult = await _client
        .from('room_products')
        .insert({
          'hotel_id': hotelId,
          'room_id': roomId,
    ...
    if (deductFromHousekeeperId != null) {
      try {
        await _client.rpc(
          'use_housekeeper_stock_for_room',
          params: {
      ...
      await _auditService.logAction(
        'Used housekeeper stock for room placement',
        details: {
  ...
  Future<void> removeProductFromRoom({required String roomProductId}) async {
    ...
    if (roomProd != null) {
      hotelId =
          roomProd['hotel_id'] != null ? asString(roomProd['hotel_id']) : null;
      roomNumber = roomProd['room_number'] != null
          ? asString(roomProd['room_number'])
          : null;
      productName =
          roomProd['name_en'] != null ? asString(roomProd['name_en']) : null;
  ...
  Future<void> updateHotelExpressQrEnabled({
    ...
    await _client
        .from('hotels')
        .update({'express_qr_enabled': enabled}).eq('id', hotelId);
