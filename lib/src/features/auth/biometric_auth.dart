...
Future<void> saveLoginCredentials(String email, String password) async {
  ...
  await secureStorage.write(key: AuthPrefs.passwordKey(trimmed), value: password);

  // Scrub legacy plaintext password if it exists
...
Future<String?> savedPasswordFor(String email) async {
  const secureStorage = FlutterSecureStorage();
  String? password = await secureStorage.read(key: AuthPrefs.passwordKey(email));

  // Fallback to reading legacy plaintext password, and migrate it to secure storage if found
...
Future<bool> hasBiometricCredentials() async {
  ...
  final password = await secureStorage.read(key: AuthPrefs.passwordKey(account));
