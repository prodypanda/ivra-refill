...
class _OperationsAnalyticsPanel extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final theme = Theme.of(context);
    final refillEvents =
        ref.watch(refillEventsProvider).valueOrNull ?? const <RefillEvent>[];
    final roomProducts =
        ref.watch(roomProductsProvider).valueOrNull ?? const <RoomProduct>[];
    final inventory =
    ...
    final visibleEvents = isStaff && currentUserId != null
        ...
        : refillEvents;
    final refillOnly = visibleEvents
    ...
    final weekly = refillOnly
        .where((e) => !e.occurredAt.isBefore(weekStart))
        .length;
    final monthly = refillOnly
    ...
    for (final event in refillOnly) {
      ...
      final productName = roomProduct.product.label(
              ...
              builder: (context, constraints) {
                ...
                final cards = [
                  _AnalyticsListCard(
                    ...
                    rows: _topRows(productUsage),
                  ),
                  _AnalyticsListCard(
                    title: 'Usage by floor',
                  ...
                  _AnalyticsListCard(
                    ...
                    rows: forecasts.isEmpty
                        ? const [MapEntry('No stock data', '')]
                        : forecasts.take(5).toList(),
                  ),
                  _AnalyticsListCard(
                    title: 'Unusual patterns',
                    icon: Icons.warning_amber_outlined,
                    rows: attentionRooms.length > 8
                        ? [
                            MapEntry(
                              '${attentionRooms.length} rooms require review',
                              'High',
                            ),
                          ]
                        : const [MapEntry('No unusual patterns detected', '')],
                  ),
                ];
                if (!wide)
                  return Column(
