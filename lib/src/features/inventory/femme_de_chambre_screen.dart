...
class FemmeDeChambreScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<FemmeDeChambreScreen> createState() =>
      _FemmeDeChambreScreenState();
}
...
class _FemmeDeChambreScreenState extends ConsumerState<FemmeDeChambreScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (currentUser == null) {
      return PageScaffold(
        ...
        child: const Center(child: PremiumLoadingWidget()),
      );
    }
  ...
  Widget _buildHousekeeperView(BuildContext context, UserProfile currentUser) {
    ...
    return AsyncValueView(
      ...
      builder: (allocations) {
        ...
        final totalFullBottles = allocations.fold<int>(
          0,
          (sum, item) => sum + item.fullBottles,
        );
        final totalEmptyBottles = allocations.fold<int>(
          0,
          (sum, item) => sum + item.emptyBottles,
        );
        final totalFullBidons = allocations.fold<int>(
          0,
          (sum, item) => sum + item.fullBidons,
        );
        final totalOpenBidons = allocations.fold<int>(
          0,
          (sum, item) => sum + item.openBidons,
        );
  ...
  Widget _buildHousekeeperExpandableCard(
          ...
          child: GestureDetector(
            ...
            child: Stack(
              children: [
                CircleAvatar(
                  backgroundColor: theme.colorScheme.primaryContainer,
                  backgroundImage: (hk.avatarUrl != null &&
                          hk.avatarUrl!.isNotEmpty &&
                          hk.avatarUrl!.startsWith('http'))
                      ? NetworkImage(hk.avatarUrl!)
                      : null,
                  child: (hk.avatarUrl == null ||
                          hk.avatarUrl!.isEmpty ||
                          !hk.avatarUrl!.startsWith('http'))
                      ? Icon(
                          Icons.person,
                          color: theme.colorScheme.onPrimaryContainer,
                    ...
                    PopupMenuButton<String>(
                        ...
                        if (value == 'toggle_status') {
                          ...
                          if (!isActive) {
                            final activeAllocations =
                                await _getActiveAllocations(hk.id);
                            if (activeAllocations.isNotEmpty &&
                              ...
                              final result = await showDialog<String>(
                                ...
                                builder: (context) => AlertDialog(
                                  title: Text(
                                    l10n.t('hkDeactivateWithStockTitle'),
                                  ),
                                  content: Text(
                                    l10n.t('hkDeactivateWithStockMessage'),
                                  ),
                                  actions: [
                                    TextButton(
                                      onPressed: () =>
                          ...
                          List<HousekeeperAllocation> activeAllocations =
                              await _getActiveAllocations(hk.id);
                          String messageKey = activeAllocations.isNotEmpty
                          ...
                          final confirm = await showDialog<bool>(
                            ...
                            builder: (c) => PremiumConfirmDialog(
                              ...
                              message: l10n.tParams(messageKey, {
                                'userName': hk.fullName,
                              }),
                      ...
                      itemBuilder: (context) => [
                        PopupMenuItem(
                          ...
                          child: Text(
                            hk.isActive
                                ? l10n.t('teamDeactivate')
                                : l10n.t('teamReactivate'),
                          ),
                        ),
                        PopupMenuItem(
                          value: 'delete',
                          child: Text(
                            l10n.t('deleteGeneric'),
        ...
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(
                  l10n.t('basketContent'),
                  style: theme.textTheme.titleSmall,
                ),
                const SizedBox(height: 8),
                Consumer(
                  builder: (context, ref, _) {
                    final basketAsync = ref.watch(
                      housekeeperBasketProvider(hk.id),
                    );
                    return basketAsync.when(
                      loading: () => const Padding(
                      ...
                      data: (basket) {
                        ...
                        return Column(
                          children: basket
                              .map(
                                (a) => _buildAllocationCard(context, a, hk.id),
                              )
                              .toList(),
                        );
                      },
              ...
              children: [
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    labelText: l10n.t('accountFullName'),
                  ),
                  validator: (v) =>
  ...
  Future<void> _showAllHistoryDialog(
    BuildContext context,
    String housekeeperId,
  ) async {
    final l10n = AppLocalizations.of(context);
          ...
          content: SizedBox(
            ...
            child: Consumer(
              builder: (context, ref, child) {
                final targetEventsAsync = ref.watch(
                  housekeeperHistoryProvider(housekeeperId),
                );
                  ...
                  data: (targetEvents) {
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final displayLabel = event.roomNumber != null &&
                                event.roomNumber!.isNotEmpty
                            ? (event.eventType ==
                                    HousekeeperStockEventType.roomPlacement
                                ? '${meta.label} ${event.roomNumber}'
                                : (event.eventType ==
                                            HousekeeperStockEventType
                                                .refillUse ||
                                        event.eventType ==
                        ...
                        return ListTile(
                          ...
                          title: Text(
                            l10n.tParams('productEventTitle', {
                              'productName': pName,
  ...
  Widget _buildAllocationCard(
    BuildContext context,
    HousekeeperAllocation allocation,
    String housekeeperId,
  ) {
    final theme = Theme.of(context);
    ...
    final openBidonPercentage =
        allocation.openBidons > 0 && allocation.product.bidonVolumeMl > 0
            ? (allocation.openBidonVolumeLeftMl /
                    allocation.product.bidonVolumeMl *
                    100)
                .clamp(0.0, 100.0)
            : 0.0;
                        ...
                        children: [
                          HoverImageTooltip(
                            ...
                            child: CircleAvatar(
                              ...
                              backgroundImage: allocation.product.imageUrl !=
                                          null &&
                                      allocation.product.imageUrl!.isNotEmpty
                                  ? NetworkImage(allocation.product.imageUrl!)
                                  : null,
                              child: allocation.product.imageUrl == null ||
                                      allocation.product.imageUrl!.isEmpty
                                  ? const Icon(
                          ...
                          IconButton(
                            tooltip: AppLocalizations.of(
                              context,
                            ).t('housekeeperStockHistory'),
                            icon: Icon(
                              Icons.history_rounded,
                              color: theme.colorScheme.primary,
                            ),
                            onPressed: () => _showProductHistoryDialog(
                              context,
                              allocation.product,
                              housekeeperId,
                            ),
                    ...
                    children: [
                      HoverImageTooltip(
                        ...
                        child: CircleAvatar(
                          ...
                          backgroundImage:
                              allocation.product.imageUrl != null &&
                                      allocation.product.imageUrl!.isNotEmpty
                                  ? NetworkImage(allocation.product.imageUrl!)
                                  : null,
                          child: allocation.product.imageUrl == null ||
              ...
              children: [
                ...
                _buildMiniDetail(
                  ...
                  label: AppLocalizations.of(
                    context,
                  ).t('inventoryTableEmptyBottlesGeneric'),
                ...
                if (allocation.product.isRefillable) ...[
                  _buildMiniDetail(
                    ...
                    label: AppLocalizations.of(
                      context,
                    ).t('inventoryTableFullBidonsGeneric'),
  ...
  Future<void> _showCheckoutDialog(BuildContext context) async {
    ...
    try {
      ...
      if (context.mounted) {
        PremiumSnackbar.show(context, l10n.t('errorGeneric'), isError: true);
      }
      return;
    }
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      onChanged: (val) {
                        if (val != null) {
                          setDialogState(() {
                            ...
                            fullBottles = fullBottles.clamp(
                              0,
                              stock?.fullBottles ?? 0,
                            );
                            fullBidons = fullBidons.clamp(
                              0,
                              stock?.fullBidons ?? 0,
                            );
                          });
                    ...
                    _buildCounterRow(
                      ...
                      max: maxBottles,
                      onChanged: (val) =>
                          setDialogState(() => fullBottles = val),
                    ),
  ...
  Future<void> _showReturnDialog(BuildContext context) async {
    ...
    if (allocations.isEmpty) {
      if (context.mounted) {
        PremiumSnackbar.show(context, l10n.t('noAllocations'), isError: true);
      }
      return;
    }
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            final theme = Theme.of(context);
            final maxVolume =
                selectedAllocation.product.bidonVolumeMl.toDouble();
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildCounterRow(
                        ...
                        onChanged: (val) =>
                            setDialogState(() => emptyBottles = val),
                      ),
                      const SizedBox(height: 16),
                      ...
                      if (selectedAllocation.product.isRefillable) ...[
                        ...
                        _buildCounterRow(
                          ...
                          onChanged: (val) =>
                              setDialogState(() => emptyBidons = val),
                        ),
  ...
  // ============================================================
  Future<void> _showProductHistoryDialog(
    BuildContext context,
    Product product,
    String housekeeperId,
  ) async {
            ...
            child: Consumer(
              builder: (context, ref, _) {
                final eventsAsync = ref.watch(
                  housekeeperStockEventsProvider((
                    housekeeperId: housekeeperId,
