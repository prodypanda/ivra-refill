-- Migration: Add proof_photo_url to refill_events and support it in approve_change_request
-- Date: 2026-06-27

-- 1. Add proof_photo_url column to refill_events table
alter table refill_events add column if not exists proof_photo_url text;

-- 2. Add foreign key from refill_events(performed_by) to profiles(id)
do $$
begin
  if not exists (
    select 1
    from information_schema.table_constraints
    where constraint_name = 'refill_events_performed_by_profile_fkey'
      and table_name = 'refill_events'
  ) then
    alter table refill_events
      add constraint refill_events_performed_by_profile_fkey
      foreign key (performed_by) references profiles(id);
  end if;
end $$;

-- 3. Update profiles select policy to allow all authenticated users to read names/emails of team members (necessary to show names in history)
drop policy if exists "profiles_select" on profiles;
create policy "profiles_select" on profiles for select using (
  auth.uid() is not null
);

-- 4. Update approve_change_request to support proof_photo_url
    ...
    begin
      ...
      if v_old_status is not null and v_new_status is not null and v_old_status <> v_new_status then
        ...
        select 
          ...
          v_request.new_data->>'proof_photo_url'
        from room_products
        where id = v_request.target_id::uuid;
      end if;
    end;
