-- Migration: Percentage-Based Inventory Refill Calculations
-- Add open_bidon_volume_left_ml column and update record_refill / undo_refill RPCs

ALTER TABLE hotel_inventory ADD COLUMN IF NOT EXISTS open_bidon_volume_left_ml double precision NOT NULL DEFAULT 0.0;

-- Initialize existing records where open_bidons is active but the volume left column is still 0
UPDATE hotel_inventory hi
SET open_bidon_volume_left_ml = COALESCE(
    (SELECT bidon_volume_ml FROM products p WHERE p.id = hi.product_id),
    5000.0
)
WHERE hi.open_bidons > 0 AND hi.open_bidon_volume_left_ml = 0.0;

DROP FUNCTION IF EXISTS record_refill(uuid, text, text);
DROP FUNCTION IF EXISTS undo_refill(uuid, text);

-- Recreate record_refill function
  ...
  IF v_product.refill_type = 'refillable' THEN
    ...
    IF FOUND THEN
      ...
      IF v_current_volume_left > v_bidon_volume_ml THEN
        v_current_volume_left := v_bidon_volume_ml; -- Cap it if we cannot roll back further
      END IF;
