...
void main() {
  group('Housekeeper Role & Allocations Tests', () {
    ...
    setUp(() async {
      ...
      final housekeeper = team.firstWhere(
        (member) => member.email == housekeeperEmail,
      );
      housekeeperId = housekeeper.id;
    });

    test(
      'Housekeeper checkout stock successfully transfers central stock to allocations',
      () async {
        // Get initial central inventory
        final initialInventory = await repository.inventory(hotelId: hotelId);
        final initialProductStock = initialInventory.firstWhere(
        ...
        final updatedProductStock = updatedInventory.firstWhere(
          (stock) => stock.product.id == productId,
        );
        expect(
          updatedProductStock.fullBottles,
          initialProductStock.fullBottles - checkoutBottles,
        );
        expect(
          updatedProductStock.fullBidons,
          initialProductStock.fullBidons - checkoutBidons,
        );
    ...
    test(
      'Housekeeper checkout stock fails if insufficient central stock',
      () async {
        final initialInventory = await repository.inventory(hotelId: hotelId);
        final initialProductStock = initialInventory.firstWhere(
          (stock) => stock.product.id == productId,
    ...
    test(
      'Room refill as housekeeper deducts from checked-out allocations rather than central inventory',
      () async {
        ...
        final rp = roomProducts.firstWhere(
        ...
        final postRefillInventory = await repository.inventory(
          hotelId: hotelId,
        );
        final postRefillProductStock = postRefillInventory.firstWhere(
          (stock) => stock.product.id == productId,
        );
        expect(
          postRefillProductStock.fullBidons,
          initialProductStock.fullBidons,
        );
        expect(
          postRefillProductStock.openBidonVolumeLeftMl,
          initialProductStock.openBidonVolumeLeftMl,
        );
    ...
    test(
      'Room bottle replacement as housekeeper deducts from allocations and tracks empty bottles',
      () async {
    ...
    test(
      'Housekeeper returning remaining stock merges and aggregates volumes back to central inventory',
      () async {
        ...
        final allocsBefore = await repository.fetchHousekeeperAllocations(
          housekeeperId: housekeeperId,
          hotelId: hotelId,
        );
        ...
        final centralStockBefore = centralBefore.firstWhere(
          (stock) => stock.product.id == productId,
        );
        ...
        final allocsAfter = await repository.fetchHousekeeperAllocations(
          housekeeperId: housekeeperId,
          hotelId: hotelId,
        );
        final allocAfter = allocsAfter.firstWhere(
        ...
        final centralStockAfter = centralAfter.firstWhere(
          (stock) => stock.product.id == productId,
        );

        expect(
          centralStockAfter.fullBottles,
          centralStockBefore.fullBottles + 1,
        );
        expect(
          centralStockAfter.emptyBottles,
          centralStockBefore.emptyBottles + 1,
        );
        ...
        final expectedTotalOpenVolume =
            centralStockBefore.openBidonVolumeLeftMl + 3000.0;
        final expectedExtraFullBidons =
            (expectedTotalOpenVolume / 5000.0).floor();
        final expectedRemainingOpenVolume =
            expectedTotalOpenVolume - (expectedExtraFullBidons * 5000.0);

        expect(
          centralStockAfter.fullBidons,
          centralStockBefore.fullBidons + 1 + expectedExtraFullBidons,
        );
        expect(
          centralStockAfter.openBidonVolumeLeftMl,
          expectedRemainingOpenVolume,
        );
    ...
    test(
      'Can add product to room and remove product from room on repository',
      () async {
        final initialRoomProducts = await repository.roomProducts();
        final r101ProductsBefore =
            initialRoomProducts.where((rp) => rp.roomId == 'room-101').toList();
        final initialCount = r101ProductsBefore.length;

        expect(
          r101ProductsBefore.any((rp) => rp.product.id == 'prod-conditioner'),
          false,
        );
        ...
        final r101ProductsAfterAdd = roomProductsAfterAdd
            .where((rp) => rp.roomId == 'room-101')
            .toList();
        expect(r101ProductsAfterAdd.length, initialCount + 1);

        final addedProduct = r101ProductsAfterAdd.firstWhere(
          (rp) => rp.product.id == 'prod-conditioner',
        );
        ...
        final r101ProductsAfterRemove = roomProductsAfterRemove
            .where((rp) => rp.roomId == 'room-101')
            .toList();
        expect(r101ProductsAfterRemove.length, initialCount);
        expect(
          r101ProductsAfterRemove.any(
            (rp) => rp.product.id == 'prod-conditioner',
          ),
          false,
        );
