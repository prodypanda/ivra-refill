...
void main() {
  group('Inventory Enforcement Tests', () {
    ...
    test('replaceBottle fails when stock is 0 and autoAdjust is false',
        () async {
      ...
      final stockIndex = inventory.indexWhere(
        (stock) => stock.product.id == rp.product.id,
      );
      if (stockIndex != -1) {
      ...
      final finalStock = updatedInventory.firstWhere(
        (stock) => stock.product.id == rp.product.id,
      );
    ...
    test(
      'replaceBottle succeeds and auto-adjusts when stock is 0 and autoAdjust is true',
      () async {
        final roomProducts = await repository.roomProducts();
        final rp = roomProducts.first;
        ...
        final updatedInventory = await repository.inventory(
          hotelId: rp.hotelId,
        );
        final finalStock = updatedInventory.firstWhere(
          (stock) => stock.product.id == rp.product.id,
        ...
        final events = await repository.recentInventoryEvents(
          hotelId: rp.hotelId,
        );
        expect(
          events.any(
            (e) =>
                e.reason == 'Auto-adjusted for replacement' &&
                e.fullBottlesDelta == 1,
          ),
          true,
        );
      },
    ...
    test('createRoomsFromTemplate enforces inventory stock checks', () async {
      ...
      expect(
        events.any(
          (e) =>
              e.reason == 'Auto-adjusted for room creation template' &&
              e.fullBottlesDelta == 5,
        ),
        true,
      );
    });
    ...
    test('createRoomsFromTemplate succeeds with empty productIds', () async {
      ...
      final newRooms =
          finalRooms.where((room) => room.floorNumber == 10).toList();
