...
void main() {
  ...
  group('QR Deep Linking and Action Overlay Tests', () {
    testWidgets(
      'Guest scans QR link with SKU -> redirects to PublicProductScreen',
      (tester) async {
        await _pumpIvraApp(
          tester,
    ...
    testWidgets('Guest scans QR link without SKU -> redirects to LoginScreen', (
      tester,
    ) async {
      await _pumpIvraApp(
        tester,
    ...
    testWidgets(
      'Authorized staff scans QR link with SKU -> opens QrActionScreen modal overlay',
      (tester) async {
        await _pumpIvraApp(
          tester,
    ...
    testWidgets(
      'Authorized staff scans QR link without SKU -> redirects to RoomsScreen with filters',
      (tester) async {
        await _pumpIvraApp(
          tester,
          currentUser: _userForRole(UserRole.hotelManager),
    ...
    testWidgets(
      'Unauthorized staff scans QR link with SKU -> shows permission warning, buttons disabled',
      (tester) async {
        // User belongs to hotel-beachfront, so they are unauthorized for hotel-seaside!
        await _pumpIvraApp(
          tester,
          currentUser: _userForRole(
            UserRole.hotelStaff,
            hotelId: 'hotel-beachfront',
          ),
        );
        ...
        final refillButton = tester.widget<FilledButton>(
          find.byKey(const ValueKey('refill_button')),
        );
      ...
      overrides: [
        ...
        if (currentUser != null)
          ...
          currentUserProvider.overrideWith(
            (ref) => Future.error(StateError('Not logged in')),
          ),
